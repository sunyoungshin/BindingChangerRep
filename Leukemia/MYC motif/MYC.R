...
#input for MEME-ChIP
MYC_bed<-read.table("ENCFF001VQK.bed/NB4_Myc_narrowPeak.bed", header = FALSE,stringsAsFactors = F, sep = "\t")
...
motif <- new("pfm", mat=as.matrix(p1), name="MYC1")
plotMotifLogo(motif)
plotMotifLogo(p1,xlab="")


p2<-t(myMYC$MYC2)
rownames(p2)<-c("A","C","G","T")
motif <- new("pfm", mat=as.matrix(p2), name="MYC2")
plotMotifLogo(motif)
plotMotifLogo(p2,xlab="")

p3<-t(myMYC$MYC3)
rownames(p3)<-c("A","C","G","T")
motif <- new("pfm", mat=as.matrix(p3), name="MYC3")
plotMotifLogo(motif)
plotMotifLogo(p3,xlab="")
